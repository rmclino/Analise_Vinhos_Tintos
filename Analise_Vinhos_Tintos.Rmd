...
11 - alcohol (potassium sulphate - g / dm3) - % de alcool no vinho
...
Acidez Fixa + Volatiol -> Vamos somar as duas e ver se elas podem ser usadas em conjunto.
...
#for(name in names(df_vinho)){
#  
#  print( paste(name, " x quality:", cor(df_vinho$quality, as.numeric(df_vinho[,name])) ))
#}
```
...
m1 <- lm(I(quality) ~ I(alcohol.trans), data= df_vinho.sub)
...
Pelos dados que temos podemos notar que a qualidade de um vinho tinto não depende exclusivamente de 1 ou poucas variaveis. Assim como é muito dificil prever a qualidade de um vinho somente pela propriedades quimicas que temos nos nossos atributos. Talvez quando comparado entre safras diferentes ou mesmo tipo de uva esses atributos possom cria um modelo com boa chances de uma previsão acertada.
Seria interessante ter mais dados e mais atributos. O preço também é um determinante importante. Como foi feito a qualificação dos vinhos pode influenciar bastante no resultado.
O interessante do projeto foi rever todos os conceitos dado no curso de EDA.
Acho que não fui feliz no dataset para construir um modelo preditivo, um R^2 muito baixo (0,34), o modelo poderia ser usado para prever a qualidade contraria com melhor propriedade.