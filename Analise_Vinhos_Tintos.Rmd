...
for( i in 1:3){ 
  nam <- paste("p",i,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= fixed.acidity,
...
for( i in 1:3){ 
  nam <- paste("p",i+3,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]),  x= volatile.acidity,
                    color = I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines"),
                    xlim =c(floor(min(df_vinho$volatile.acidity)),ceiling(max(df_vinho$volatile.acidity))))
 )}
#citric.acid
for( i in 1:3){ 
  nam <- paste("p",i+6,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]),x= citric.acid, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines"),
...
for( i in 1:3){ 
  nam <- paste("p",i,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]),x= total.acidity,
                    color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines"),
                    xlim = c(floor(min(df_vinho$total.acidity)),ceiling(max(df_vinho$total.acidity)))
                    )
         )
}
...
df_vinho %>%
  group_by(quality.group) %>%
  summarise( Mean = mean(total.acidity), Median = median(total.acidity), DevPad=sd(total.acidity))
```
...
for( i in 1:3){ 
  nam <- paste("p",i,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= residual.sugar, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i+3,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= chlorides, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= free.sulfur.dioxide, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i+3,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= total.sulfur.dioxide, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i+6,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= sulphates, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
df_vinho$fixed.sulfur.dioxide <- log( df_vinho$total.sulfur.dioxide - df_vinho$free.sulfur.dioxide)
#fixed.sulfur.dioxide
for( i in 1:3){ 
  nam <- paste("p",i+0,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= fixed.sulfur.dioxide, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
            xlim = c(floor(min(df_vinho$fixed.sulfur.dioxide)),ceiling(max(df_vinho$fixed.sulfur.dioxide))))))
}
grid.arrange(p1,p2,p3,ncol=3)
...
for( i in 1:3){ 
  nam <- paste("p",i+0,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= density, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i+3,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= pH, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
...
for( i in 1:3){ 
  nam <- paste("p",i+6,sep="")
  assign(nam, qplot(data=subset(df_vinho, quality.group == groups[i]), x= alcohol.trans, 
            color =I('black'), fill=I(colors[i]), main = paste(groups[i]," Qualy Wines",
            xlim = c((min(df_vinho$alcohol.trans)),(max(df_vinho$alcohol.trans))))))
}
grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p9, ncol=3)
```
...
ggplot(data=df_vinho.sub, aes(x=alcohol.trans, y= sulphates, color= quality.group))+
  geom_boxplot()+
...
ggplot(df_vinho.sub, aes(alcohol.trans))+
  ...
  geom_vline(xintercept = median(df_vinho.sub$alcohol.trans), color = 'darkblue') +
  annotate('text',
           x = median(df_vinho.sub$alcohol.trans) - 0.15,
           y = 500,
           label = paste('median\n(', round(median(df_vinho.sub$alcohol.trans),3), ')', sep = ''),
           color = 'darkblue') +
  geom_vline(xintercept = mean(df_vinho.sub$alcohol.trans), color = I('darkgreen')) +
  annotate('text',
           x = mean(df_vinho.sub$alcohol) + 0.1,
           y = 500,
           label = paste('mean\n(', round(mean(df_vinho.sub$alcohol.trans), 3), ')', sep = ''),
           color = 'darkgreen') +
  ggtitle("Distribuição do Logaritimo do Percentual de Alcool presente nos Vinhos")
...
ggplot( data = df_vinho.sub, aes(x=fixed.acidity, y=citric.acid)) +
  ...
  scale_y_continuous(limits= c(min(df_vinho.sub$citric.acid),quantile(df_vinho.sub$citric.acid,0.99)))+
...
ggplot( data = df_vinho.sub, aes(x=sulphates, y=citric.acid, color=quality.group))+
  geom_jitter(alpha=1/2) +
...
Acho que não fui feliz no dataset para construir um modelo preditivo, um R^2 muito baixo (0,347), o modelo poderia talvez ser usado para prever a qualidade contraria com melhor propriedade.